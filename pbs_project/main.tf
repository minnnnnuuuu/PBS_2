...
resource "null_resource" "gateway_api_crds" {
  provisioner "local-exec" {
    # 윈도우 환경이므로 PowerShell을 사용하고, 접속 정보를 먼저 갱신합니다.
    command = <<EOT
      aws eks update-kubeconfig --region ap-northeast-2 --name ${module.eks.cluster_name}

      # 2. [중요] DNS 전파 대기 (30초~1분 정도 기다려야 'no such host' 에러가 안 납니다)
      Write-Host "Waiting for EKS DNS propagation..."
...
output "route53_zone_id" {
  description = "Route53 Hosted Zone ID"
  value       = module.route53.zone_id  # 모듈의 output을 참조
}

output "route53_nameservers" {
  description = "Route53 Name Servers"
  value       = module.route53.name_servers # 모듈에 name_servers output이 있다면 연결
}

# =================================================================
# 12. 추가 필수 리소스 (한 방 배포용)
# =================================================================

# 1. Pod 생성을 막았던 서비스 어카운트 생성
resource "kubernetes_service_account" "hybrid_ai_sa" {
  metadata {
    name      = "hybrid-ai-sa"
    namespace = "default"
    annotations = {
      # IAM Role을 연결해줘야 Pod가 EFS나 ECR에 접근할 수 있습니다.
      "eks.amazonaws.com/role-arn" = module.iam.eks_node_role_arn
    }
  }
  # EKS 클러스터가 완전히 준비된 후에 생성되도록 설정
  depends_on = [module.eks]
}

# 2. ArgoCD 설치 (이게 있어야 ingress.tf에서 에러가 안 납니다)
resource "helm_release" "argocd" {
  ...
  # EKS와 로드밸런서 컨트롤러가 준비된 후 설치
  depends_on = [module.eks, helm_release.aws_lbc]
}

# 3. ACM 모듈 (ingress.tf에서 인증서 주소를 참조할 때 필요)
module "route53" { # 이름을 route53_acm에서 route53으로 통일
  source       = "./modules/route53" # 폴더명에 맞게 수정
  project_name = var.project_name
  domain_name  = "test.cloudreaminu.cloud" # 모듈이 요구하는 변수명으로 수정
}